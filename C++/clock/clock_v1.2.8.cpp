...
List of "things":
	...
	5) A strategy that seems rewarding was the get_ONES() and get_TENS() methods, along with the get_INT(), get_ASCII(),
	   and convert_ASCII() methods, to efficiently print numbers.
		...
		The clock is now working as intentend. The program is now reading/writing to file properly and all other machinery is
		working.

		Our next step is to find a better way to calculate ERROR and using that change ThisTime in a smooth way.

		Id also like to offload version control to github.

		(2/14) ASSIDE:	
		
		I have no made any updates to the code. I wanted to make a list of the things Im still hoping to learn and 
		apply to this project:
		
			1) Github (Version control)

			2) ROOT (Data analysis in C++)

			3) Error Propogation

		I started looking at networking, but I think I have a lot to learn before networking becomes something I can
		understand on a level I deem sufficient.
...
void compare_TIME( int time1, int time2 ) {
	...
	ERROR= sqrt((pow(time1, 2) + pow(time2, 2)) / (2 * pow(time2, 2)));
	...
	if ( ThisTime < 10000 ) {
		...
		ThisTime= ThisTime - (ERROR * Learning_Rate);

		}

	else {

		ThisTime= ThisTime + (1 / ERROR * Learning_Rate);
